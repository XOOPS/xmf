...
PHPStan runs at level `max`. The baseline (`phpstan-baseline.neon`) tracks ~586 existing errors for incremental cleanup. New code must not introduce new PHPStan errors. Run `composer baseline` to regenerate after intentionally resolving baseline items. The baseline script backs up the current file before regenerating and restores it on failure.
